...
struct Settings
{
  ...
  ParamsUBO toParamsUBO () const
  {
    ...
    return {
      // Material
      ior,          tintAmount,             maxDepth,
      tintR,        tintG,                  tintB,
      // Sun
      sunEnabled  ? 1 : 0,
      (sunShadows  && shadowsEnabled)  ? 1 : 0,
      (sunCaustics && causticsEnabled) ? 1 : 0,
      sunAzimuth,   sunElevation,           sunIntensity,  sunConeHalf,
      sunDiskExp,   sunCoronaExp,
      sunDiskR,     sunDiskG,               sunDiskB,
      sunCoronaR,   sunCoronaG,             sunCoronaB,
      // Point light
      pointEnabled ? 1 : 0,
      (pointShadows  && shadowsEnabled)  ? 1 : 0,
      (pointCaustics && causticsEnabled) ? 1 : 0,
      pointLightX,  pointLightY,            pointLightZ,  pointLightRadius,
      pointLightR,  pointLightG,            pointLightB,  pointLightIntensity,
      // Surface
      (int)surfaceType,  sphereRadius,  sphereHeight,  cubeSize,
      // Rendering
      nCaustics,    ambient,               floorZ,
      // Floor
      floorScale,
      floorLightR,  floorLightG,           floorLightB,
      floorDarkR,   floorDarkG,            floorDarkB,
      // Sky
      skyHorizonR,  skyHorizonG,           skyHorizonB,
      skyZenithR,   skyZenithG,            skyZenithB,
      // Blobbies
      b1x,          blobsCenterY,          blobsCenterZ,  blob1W,
      blob1Mu,      blob1Sigma,            blob1SigmaW,
      b2x,          blobsCenterY,          blobsCenterZ,  blob2W,
      blob2Mu,      blob2Sigma,            blob2SigmaW,
      blobbiesThreshold,
      // Shadows
      shadowSamples,   shadowSoftness,
      // Caustics
      causticDiskScale, causticFalloff, causticBlendRadius, causticDitherAmt,
      // Blobby quality
      blobMarchSteps,
    };
  }
