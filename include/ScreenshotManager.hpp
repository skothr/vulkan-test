...
class ScreenshotManager
{
  ...
  void init(const VkCtx &ctx, VkFormat scFmt, VkExtent2D ext, const std::vector<VkImage> &scImgs, VkImage storageImg);
  void onSwapchainRecreate(VkFormat scFmt, VkExtent2D ext, const std::vector<VkImage> &scImgs, VkImage storageImg);
  void cleanup();
  ...
  void closePopup()
  {
    if (popupVisible) { requestClose = true; }
  }
  ...
  void        createImage(uint32_t w, uint32_t h, VkFormat fmt, VkImageUsageFlags usage, VkImage &img, VkDeviceMemory &mem);
  VkImageView createImageView(VkImage img, VkFormat fmt);
  VkCommandBuffer beginOneTimeCmd();
  void            endOneTimeCmd(VkCommandBuffer cb);
  void transitionImage(VkCommandBuffer cb, VkImage img, VkImageLayout oldLayout, VkImageLayout newLayout, VkAccessFlags srcAccess,
