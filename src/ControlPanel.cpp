...
ControlPanel::ControlPanel(Settings &settings)
    : settings(settings)
{}
...
void ControlPanel::createFramebuffers(VkDevice dev, const std::vector<VkImageView> &views, VkExtent2D extent)
{
...
void ControlPanel::init(GLFWwindow *win, VkInstance inst, VkPhysicalDevice physDev, VkDevice dev, uint32_t queueFamily, VkQueue queue,
                        VkCommandPool pool, VkFormat scFormat, const std::vector<VkImageView> &scViews, VkExtent2D extent,
...
void ControlPanel::onSwapchainRecreate(VkDevice dev, VkFormat scFormat, const std::vector<VkImageView> &scViews, VkExtent2D extent)
{
...
void ControlPanel::drawSurfaceSection()
{
  ...
  ImGui::RadioButton("Sphere", &type, (int)SurfaceType::SPHERE);
  ImGui::SameLine();
  ImGui::RadioButton("Cube", &type, (int)SurfaceType::CUBE);
  ImGui::SameLine();
...
void ControlPanel::drawSunSection()
{
  ...
  if (settings.sunEnabled)
  {
    ...
    ImGui::Checkbox("Shadows##sun", &settings.sunShadows);
    ImGui::SameLine();
...
void ControlPanel::drawPointLightSection()
{
  ...
  if (settings.pointEnabled)
  {
    ...
    ImGui::Checkbox("Shadows##pt", &settings.pointShadows);
    ImGui::SameLine();
...
void ControlPanel::drawImGuiColorsSection()
{
  ...
  for (int i = 0; i < ImGuiCol_COUNT; i++)
  {
    ...
    ImGui::PushID(i);
    ImGui::ColorEdit4("##color", (float *)&s.Colors[i], ImGuiColorEditFlags_AlphaBar | ImGuiColorEditFlags_AlphaPreviewHalf);
...
void ControlPanel::drawCaptureSection()
{
  ...
  if (ImGui::InputText("##suffix", buf, sizeof(buf))) { settings.screenshotSuffix = buf; }
...
void ControlPanel::drawDebugOverlay(const DebugInfo &dbg)
{
  ...
  {
    ImGui::Separator();
    ImGui::Text("Camera  θ %.2f  φ %.2f  d %.2f", dbg.camTheta, dbg.camPhi, dbg.camDist);
    ImGui::Text("IOR %.2f   Tint %.2f   Depth %d", settings.ior, settings.tintAmount, settings.maxDepth);
    ImGui::Text("Sun  az %.2f  el %.2f  int %.2f", settings.sunAzimuth, settings.sunElevation, settings.sunIntensity);
  }
...
void ControlPanel::draw(const DebugInfo &dbg)
{
  ...
  ImGui::SetNextWindowSizeConstraints(ImVec2(200.0f, 100.0f), ImVec2(FLT_MAX, io.DisplaySize.y - PAD_PANEL * 2.0f));
  ImGui::Begin("Settings", nullptr, ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse);
