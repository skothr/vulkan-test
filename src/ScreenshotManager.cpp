...
void ScreenshotManager::init(const VkCtx &c, VkFormat fmt, VkExtent2D ext, const std::vector<VkImage> &imgs, VkImage stImg)
{
  ctx        = c;
  scFmt      = fmt;
  scExt      = ext;
...
void ScreenshotManager::onSwapchainRecreate(VkFormat fmt, VkExtent2D ext, const std::vector<VkImage> &imgs, VkImage stImg)
{
  scFmt      = fmt;
  scExt      = ext;
  scImgs     = imgs;
  ...
  {
    renderNeeded              = true;
    captureCompositeNextFrame = true;
  }
}
...
void ScreenshotManager::cleanup()
{
  ...
  if (sampler)
  {
    vkDestroySampler(ctx.dev, sampler, nullptr);
    sampler = VK_NULL_HANDLE;
  }
  if (compBuf)
  {
    vkDestroyBuffer(ctx.dev, compBuf, nullptr);
    compBuf = VK_NULL_HANDLE;
  }
  if (compMem)
  {
    vkFreeMemory(ctx.dev, compMem, nullptr);
    compMem = VK_NULL_HANDLE;
  }
  inited = false;
}
...
void ScreenshotManager::update()
{
  ...
  if (compositeRequested)
  {
    ...
    createBuffer(size, VK_BUFFER_USAGE_TRANSFER_DST_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                 compBuf, compMem);
  ...
  if (optionsCapture)
  {
    ...
    if (mode == Mode::SCENE_ONLY) { capture(capturePathStr); }
...
void ScreenshotManager::drawPopups() { drawOptionsPopup(); }
...
void ScreenshotManager::recordBeforeUI(VkCommandBuffer cb)
{
  ...
  transitionImage(cb, scenePreviewImg, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
  ...
  vkCmdCopyImage(cb, storageImg, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, scenePreviewImg, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &region);

  transitionImage(cb, scenePreviewImg, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
...
void ScreenshotManager::recordAfterUI(VkCommandBuffer cb, uint32_t imgIdx)
{
  ...
  if (captureComposite)
  {
    ...
    vkCmdBlitImage(cb, scImage, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, compPreviewImg, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &blit,
                   VK_FILTER_NEAREST);
    ...
    vkCmdPipelineBarrier(cb, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT | VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT,
  ...
  if (pendingComposite && compBuf)
  {
    ...
    vkCmdPipelineBarrier(cb, VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, nullptr, 0, nullptr, 1,
                         &toSrc);

    VkBufferImageCopy copyRegion{};
    copyRegion.imageSubresource = { VK_IMAGE_ASPECT_COLOR_BIT, 0, 0, 1 };
    copyRegion.imageExtent      = { scExt.width, scExt.height, 1 };
    vkCmdCopyImageToBuffer(cb, scImage, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, compBuf, 1, &copyRegion);
...
{
  int idx = nextIndex();
  return "./images/screenshot_" + std::to_string(idx) + (suffix.empty() ? "" : "_" + suffix) + ".png";
}
...
void ScreenshotManager::capture(const std::string &path)
{
  ...
  createBuffer(size, VK_BUFFER_USAGE_TRANSFER_DST_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, stageBuf,
               stageMem);

  VkCommandBuffer cb = beginOneTimeCmd();

  transitionImage(cb, storageImg, VK_IMAGE_LAYOUT_GENERAL, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, VK_ACCESS_SHADER_WRITE_BIT,
                  VK_ACCESS_TRANSFER_READ_BIT, VK_PIPELINE_STAGE_RAY_TRACING_SHADER_BIT_KHR, VK_PIPELINE_STAGE_TRANSFER_BIT);

  VkBufferImageCopy region{};
  region.imageSubresource = { VK_IMAGE_ASPECT_COLOR_BIT, 0, 0, 1 };
  region.imageExtent      = { w, h, 1 };
  vkCmdCopyImageToBuffer(cb, storageImg, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, stageBuf, 1, &region);

  transitionImage(cb, storageImg, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, VK_IMAGE_LAYOUT_GENERAL, VK_ACCESS_TRANSFER_READ_BIT,
                  VK_ACCESS_SHADER_WRITE_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_RAY_TRACING_SHADER_BIT_KHR);
...
void ScreenshotManager::drawOptionsPopup()
{
  ...
  if (!ImGui::BeginPopupModal("Screenshot Options", nullptr, ImGuiWindowFlags_AlwaysAutoResize))
  {
  ...
  // Full-screen dim drawn inside the modal's draw list (modal layer → above all windows)
  {
    ...
    dl->AddRectFilled(wpos, { wpos.x + wsz.x, wpos.y + wsz.y }, ImGui::GetColorU32(ImGuiCol_PopupBg), style.WindowRounding);
    if (style.WindowBorderSize > 0.0f)
      dl->AddRect(wpos, { wpos.x + wsz.x, wpos.y + wsz.y }, ImGui::GetColorU32(ImGuiCol_Border), style.WindowRounding, 0,
  ...
  if (ImGui::IsItemHovered()) { ImGui::SetTooltip("Raw ray-traced output — no debug overlay or UI panels."); }
  ImGui::SameLine();
  ...
  else // CUSTOM
  {
    ...
    if (ImGui::IsItemHovered()) { ImGui::SetTooltip("Filename without extension — .png is appended automatically."); }

    std::string dir  = optionsDir[0] ? optionsDir : "./images/";
    std::string name = optionsFilename[0] ? optionsFilename : "screenshot";
    if (!dir.empty() && dir.back() != '/' && dir.back() != '\\') { dir += '/'; }
    if (name.size() > 4 && name.substr(name.size() - 4) == ".png") { name = name.substr(0, name.size() - 4); }
  ...
  ImGui::Text("Path     ");
  ImGui::SameLine();
  ImGui::TextColored(prevExists ? ImVec4{ 1.0f, 0.4f, 0.3f, 1.0f } : ImVec4{ 0.75f, 0.75f, 0.75f, 1.0f }, "%s", prevPath.c_str());
  if (prevExists)
  {
    ImGui::SameLine();
    ImGui::TextColored({ 1.0f, 0.4f, 0.3f, 1.0f }, "(exists)");
  }
  ...
  else
  {
    ...
    if (!isAuto) { ImGui::Spacing(); }
    else
    {
      // Alternate suffix input for "Save as New"
      ImGui::Spacing();
      ImGui::Text("New suffix ");
      ImGui::SameLine();
      ImGui::SetNextItemWidth(-1.0f);
      ...
      ImGui::Text("Save as   ");
      ImGui::SameLine();
      ImGui::TextColored(altExists ? ImVec4{ 1.0f, 0.4f, 0.3f, 1.0f } : ImVec4{ 0.75f, 0.75f, 0.75f, 1.0f }, "%s", altPath.c_str());
      ImGui::Spacing();
    ...
    ImGui::SameLine();
    if (ImGui::Button("Back", { 80.0f, 0.0f })) { awaitingOverwriteConfirm = false; }
    if (requestClose)
    {
      requestClose             = false;
      awaitingOverwriteConfirm = false;
...
{
  destroyScenePreview();
  createImage(scExt.width, scExt.height, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT,
              scenePreviewImg, scenePreviewMem);
  scenePreviewView = createImageView(scenePreviewImg, VK_FORMAT_R8G8B8A8_UNORM);

  VkCommandBuffer cb = beginOneTimeCmd();
  transitionImage(cb, scenePreviewImg, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, 0, VK_ACCESS_SHADER_READ_BIT,
...
void ScreenshotManager::destroyScenePreview()
{
  if (scenePreviewView)
  {
    vkDestroyImageView(ctx.dev, scenePreviewView, nullptr);
    scenePreviewView = VK_NULL_HANDLE;
  }
  if (scenePreviewImg)
  {
    vkDestroyImage(ctx.dev, scenePreviewImg, nullptr);
    scenePreviewImg = VK_NULL_HANDLE;
  }
  if (scenePreviewMem)
  {
    vkFreeMemory(ctx.dev, scenePreviewMem, nullptr);
    scenePreviewMem = VK_NULL_HANDLE;
  }
}
...
void ScreenshotManager::updateSceneDescriptor()
{
  ...
  if (!sceneDescSet)
  {
    sceneDescSet = ImGui_ImplVulkan_AddTexture(sampler, scenePreviewView, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
    return;
  }
  VkDescriptorImageInfo ii{ sampler, scenePreviewView, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL };
  VkWriteDescriptorSet  wr{ VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET };
  wr.dstSet          = sceneDescSet;
  wr.dstBinding      = 0;
...
void ScreenshotManager::createCompositePreview()
{
  ...
  VkCommandBuffer cb = beginOneTimeCmd();
  transitionImage(cb, compPreviewImg, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, 0, VK_ACCESS_SHADER_READ_BIT,
                  VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT);
  endOneTimeCmd(cb);
...
void ScreenshotManager::destroyCompositePreview()
{
  if (compPreviewView)
  {
    vkDestroyImageView(ctx.dev, compPreviewView, nullptr);
    compPreviewView = VK_NULL_HANDLE;
  }
  if (compPreviewImg)
  {
    vkDestroyImage(ctx.dev, compPreviewImg, nullptr);
    compPreviewImg = VK_NULL_HANDLE;
  }
  if (compPreviewMem)
  {
    vkFreeMemory(ctx.dev, compPreviewMem, nullptr);
    compPreviewMem = VK_NULL_HANDLE;
  }
}
...
void ScreenshotManager::updateCompositeDescriptor()
{
  ...
  {
    compPreviewDescSet = ImGui_ImplVulkan_AddTexture(sampler, compPreviewView, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
    return;
  }
  VkDescriptorImageInfo ii{ sampler, compPreviewView, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL };
  VkWriteDescriptorSet  wr{ VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET };
  wr.dstSet          = compPreviewDescSet;
  wr.dstBinding      = 0;
  wr.descriptorCount = 1;
  wr.descriptorType  = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
  wr.pImageInfo      = &ii;
...
void ScreenshotManager::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags props, VkBuffer &buf,
                                     VkDeviceMemory &mem)
{
  ...
  if (vkCreateBuffer(ctx.dev, &ci, nullptr, &buf) != VK_SUCCESS) { throw std::runtime_error("ScreenshotManager: vkCreateBuffer failed"); }
...
void ScreenshotManager::createImage(uint32_t w, uint32_t h, VkFormat fmt, VkImageUsageFlags usage, VkImage &img, VkDeviceMemory &mem)
{
  ...
  ci.sharingMode   = VK_SHARING_MODE_EXCLUSIVE;
  if (vkCreateImage(ctx.dev, &ci, nullptr, &img) != VK_SUCCESS) { throw std::runtime_error("ScreenshotManager: vkCreateImage failed"); }
  VkMemoryRequirements req;
  vkGetImageMemoryRequirements(ctx.dev, img, &req);
  VkMemoryAllocateInfo ai{ VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO };
  ai.allocationSize  = req.size;
...
void ScreenshotManager::transitionImage(VkCommandBuffer cb, VkImage img, VkImageLayout oldLayout, VkImageLayout newLayout,
