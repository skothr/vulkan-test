...
void Application::initWindow()
{
  ...
  glfwSetFramebufferSizeCallback(window,
                                 [](GLFWwindow *w, int, int) { ((Application *)glfwGetWindowUserPointer(w))->framebufferResized = true; });
  glfwSetKeyCallback(window, [](GLFWwindow *w, int key, int /*scancode*/, int action, int mods) {
    ...
    if (action == GLFW_PRESS && (!app->controlPanel.wantsKeyboard() || altKey || funcKey)) { app->keys.process(key, mods); }
    if (action == GLFW_PRESS && key == GLFW_KEY_ESCAPE) { app->screenshotMgr.closePopup(); }
  });
  ...
  glfwSetCursorPosCallback(window, [](GLFWwindow *w, double x, double y) {
    ...
    if (app->middleDown)
    {
      ...
      glm::vec3 camOff  = { app->camDist * glm::cos(app->camPhi) * glm::cos(app->camTheta),
                            app->camDist * glm::cos(app->camPhi) * glm::sin(app->camTheta), app->camDist * glm::sin(app->camPhi) };
...
{
  keys.bind(GLFW_KEY_Q, GLFW_MOD_CONTROL, "Ctrl+Q    Quit", [this]() { glfwSetWindowShouldClose(window, GLFW_TRUE); });
  ...
  keys.bind(GLFW_KEY_TAB, 0, "Tab       Toggle settings panel", [this]() { settings.showPanel = !settings.showPanel; });
  keys.bind(GLFW_KEY_I, GLFW_MOD_ALT, "Alt+I     Toggle ImGui demo window", [this]() { settings.showDemo = !settings.showDemo; });
  keys.bind(GLFW_KEY_F12, 0, "F12          Open screenshot options", [this]() { screenshotMgr.openOptions(settings.screenshotSuffix); });
  ...
  keys.bind(GLFW_KEY_F1, 0, "F1        Show key bindings", [this]() { keys.printHelp(); });
...
void Application::createInstance()
{
  ...
  if (vkCreateInstance(&ci, nullptr, &instance) != VK_SUCCESS) { throw std::runtime_error("vkCreateInstance failed"); }
}
...
void Application::createLogicalDevice()
{
  ...
  VkPhysicalDeviceBufferDeviceAddressFeaturesKHR bdaFeats{ VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_BUFFER_DEVICE_ADDRESS_FEATURES_KHR };
  ...
  if (vkCreateDevice(physDev, &ci, nullptr, &dev) != VK_SUCCESS) { throw std::runtime_error("vkCreateDevice failed"); }
...
{
  uint32_t cnt;
  vkGetPhysicalDeviceSurfaceFormatsKHR(physDev, surface, &cnt, nullptr);
...
{
  uint32_t cnt;
  vkGetPhysicalDeviceSurfacePresentModesKHR(physDev, surface, &cnt, nullptr);
...
void Application::createSwapchain()
{
  ...
  ci.presentMode    = mode;
  ci.clipped        = VK_TRUE;
  if (vkCreateSwapchainKHR(dev, &ci, nullptr, &swapchain) != VK_SUCCESS) { throw std::runtime_error("vkCreateSwapchainKHR failed"); }
...
VkImageView Application::createImageView(VkImage img, VkFormat fmt, VkImageAspectFlags aspect)
{
  ...
  VkImageView view;
  if (vkCreateImageView(dev, &ci, nullptr, &view) != VK_SUCCESS) { throw std::runtime_error("vkCreateImageView failed"); }
  return view;
}
...
void Application::createImageViews()
{
  ...
  for (size_t i = 0; i < scImages.size(); i++) { scViews[i] = createImageView(scImages[i], scFormat, VK_IMAGE_ASPECT_COLOR_BIT); }
}
...
void Application::createImage(uint32_t w, uint32_t h, VkFormat fmt, VkImageTiling tiling, VkImageUsageFlags usage,
                              VkMemoryPropertyFlags memProps, VkImage &img, VkDeviceMemory &mem)
{
  ...
  if (vkCreateImage(dev, &ci, nullptr, &img) != VK_SUCCESS) { throw std::runtime_error("vkCreateImage failed"); }
  VkMemoryRequirements req;
  vkGetImageMemoryRequirements(dev, img, &req);
  VkMemoryAllocateInfo ai{ VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO };
  ai.allocationSize  = req.size;
  ai.memoryTypeIndex = findMemoryType(req.memoryTypeBits, memProps);
  if (vkAllocateMemory(dev, &ai, nullptr, &mem) != VK_SUCCESS) { throw std::runtime_error("vkAllocateMemory failed"); }
...
void Application::createStorageImage()
{
  ...
  createImage(scExtent.width, scExtent.height, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_TILING_OPTIMAL,
              VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, storageImg, storageMem);
...
void Application::createDescriptorSetLayout()
{
  ...
  bindings[3].stageFlags      = VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR | VK_SHADER_STAGE_MISS_BIT_KHR | VK_SHADER_STAGE_INTERSECTION_BIT_KHR;
...
VkShaderModule Application::createShaderModule(const std::vector<char> &code)
{
  ...
  VkShaderModule m;
  if (vkCreateShaderModule(dev, &ci, nullptr, &m) != VK_SUCCESS) { throw std::runtime_error("vkCreateShaderModule failed"); }
  return m;
}
...
void Application::createRTPipeline()
{
  ...
  if (vkCreatePipelineLayout(dev, &pli, nullptr, &pipeLayout) != VK_SUCCESS) { throw std::runtime_error("vkCreatePipelineLayout failed"); }
...
void Application::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags props, VkBuffer &buf, VkDeviceMemory &mem,
                               bool deviceAddr)
{
  ...
  ci.usage       = usage;
  ci.sharingMode = VK_SHARING_MODE_EXCLUSIVE;
  if (vkCreateBuffer(dev, &ci, nullptr, &buf) != VK_SUCCESS) { throw std::runtime_error("vkCreateBuffer failed"); }
  VkMemoryRequirements req;
...
void Application::createAABBBuffer()
{
  ...
  AabbData aabb{ -AABB_HALF, -AABB_HALF, -AABB_HALF, AABB_HALF, AABB_HALF, AABB_HALF };

  VkDeviceSize   size = sizeof(aabb);
  VkBuffer       stageBuf;
  VkDeviceMemory stageMem;
  createBuffer(size, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, stageBuf,
               stageMem);
  void *data;
  vkMapMemory(dev, stageMem, 0, size, 0, &data);
  memcpy(data, &aabb, size);
  ...
  createBuffer(size,
               ...
               VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, aabbBuf, aabbMem, true);
...
void Application::createUniformBuffers()
{
  ...
  uboBufs.resize(MAX_FRAMES);
  uboMems.resize(MAX_FRAMES);
  uboMapped.resize(MAX_FRAMES);
  paramsBufs.resize(MAX_FRAMES);
  paramsMems.resize(MAX_FRAMES);
  paramsMapped.resize(MAX_FRAMES);
  for (int i = 0; i < MAX_FRAMES; i++)
  {
    ...
    createBuffer(paramsSize, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
...
void Application::createBLAS()
{
  ...
  pfnGetASBuildSizes(dev, VK_ACCELERATION_STRUCTURE_BUILD_TYPE_DEVICE_KHR, &buildInfo, &primCount, &sizeInfo);
  ...
  createBuffer(sizeInfo.buildScratchSize, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT,
...
void Application::createTLAS()
{
  ...
  createBuffer(instSize, VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_BUILD_INPUT_READ_ONLY_BIT_KHR | VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT,
               VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, instanceBuf, instanceMem, true);
  ...
  VkAccelerationStructureBuildSizesInfoKHR sizeInfo{ VK_STRUCTURE_TYPE_ACCELERATION_STRUCTURE_BUILD_SIZES_INFO_KHR };
  pfnGetASBuildSizes(dev, VK_ACCELERATION_STRUCTURE_BUILD_TYPE_DEVICE_KHR, &buildInfo, &instanceCount, &sizeInfo);
...
void Application::createSBT()
{
  ...
  createBuffer(sbtTotal, VK_BUFFER_USAGE_SHADER_BINDING_TABLE_BIT_KHR | VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT,
               VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, sbtBuf, sbtMem, true);

  void *mapped;
  vkMapMemory(dev, sbtMem, 0, sbtTotal, 0, &mapped);
...
void Application::createDescriptorPool()
{
  ...
  if (vkCreateDescriptorPool(dev, &ci, nullptr, &descPool) != VK_SUCCESS) { throw std::runtime_error("vkCreateDescriptorPool failed"); }
}
...
void Application::createDescriptorSets()
{
  ...
  if (vkAllocateDescriptorSets(dev, &ai, descSets.data()) != VK_SUCCESS) { throw std::runtime_error("vkAllocateDescriptorSets failed"); }
...
void Application::createCommandPool()
{
  ...
  if (vkCreateCommandPool(dev, &ci, nullptr, &cmdPool) != VK_SUCCESS) { throw std::runtime_error("vkCreateCommandPool failed"); }
}
...
void Application::createCommandBuffers()
{
  ...
  if (vkAllocateCommandBuffers(dev, &ai, cmdBufs.data()) != VK_SUCCESS) { throw std::runtime_error("vkAllocateCommandBuffers failed"); }
}

void Application::recordCommandBuffer(VkCommandBuffer cb, uint32_t imgIdx)
{
  VkCommandBufferBeginInfo bi{ VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO };
  if (vkBeginCommandBuffer(cb, &bi) != VK_SUCCESS) { throw std::runtime_error("vkBeginCommandBuffer failed"); }
  ...
  vkCmdPipelineBarrier(cb, VK_PIPELINE_STAGE_HOST_BIT, VK_PIPELINE_STAGE_ACCELERATION_STRUCTURE_BUILD_BIT_KHR, 0, 1, &hostBarrier, 0,
                       nullptr, 0, nullptr);
  ...
  vkCmdPipelineBarrier(cb, VK_PIPELINE_STAGE_ACCELERATION_STRUCTURE_BUILD_BIT_KHR, VK_PIPELINE_STAGE_RAY_TRACING_SHADER_BIT_KHR, 0, 1,
                       &asBarrier, 0, nullptr, 0, nullptr);
  ...
  if (!screenshotMgr.isPopupOpen() || screenshotMgr.isRenderNeeded())
  {
    ...
    vkCmdPipelineBarrier(cb, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, VK_PIPELINE_STAGE_RAY_TRACING_SHADER_BIT_KHR, 0, 0, nullptr, 0, nullptr, 1,
                         &toGeneral);

    // --- Trace rays ---
    vkCmdBindPipeline(cb, VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR, pipeline);
    vkCmdBindDescriptorSets(cb, VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR, pipeLayout, 0, 1, &descSets[frame], 0, nullptr);
    pfnCmdTraceRays(cb, &sbtRgen, &sbtMiss, &sbtHit, &sbtCall, scExtent.width, scExtent.height, 1);
    ...
    vkCmdPipelineBarrier(cb, VK_PIPELINE_STAGE_RAY_TRACING_SHADER_BIT_KHR, VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, nullptr, 0, nullptr, 2,
                         copyBarriers);
    ...
    vkCmdBlitImage(cb, storageImg, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, scImages[imgIdx], VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &blit,
                   VK_FILTER_NEAREST);
    ...
    vkCmdPipelineBarrier(cb, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT, 0, 0, nullptr, 0, nullptr, 1,
                         &postBlitBarrier);
  }
  else
  {
    ...
    vkCmdPipelineBarrier(cb, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT, 0, 0, nullptr, 0, nullptr, 1,
                         &toAttachment);
  }
  ...
  if (vkEndCommandBuffer(cb) != VK_SUCCESS) { throw std::runtime_error("vkEndCommandBuffer failed"); }
}
...
void Application::cleanupSwapchain()
{
  ...
  storageImg = VK_NULL_HANDLE;
  vkFreeMemory(dev, storageMem, nullptr);
  storageMem = VK_NULL_HANDLE;
...
void Application::updateUBO(uint32_t fi)
{
  ...
  glm::mat4 rot  = settings.autoRotate ? glm::rotate(glm::mat4(1.0f), t * glm::radians(90.0f) * settings.rotSpeed, axis) : glm::mat4(1.0f);
  ubo.model      = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, 0.0f, settings.sphereHeight)) * rot;
  glm::vec3 camOff = { camDist * glm::cos(camPhi) * glm::cos(camTheta), camDist * glm::cos(camPhi) * glm::sin(camTheta),
...
void Application::drawFrame()
{
  ...
  else if (res != VK_SUCCESS && res != VK_SUBOPTIMAL_KHR) { throw std::runtime_error("vkAcquireNextImageKHR failed"); }
  ...
  if (vkQueueSubmit(graphicsQ, 1, &si, inFlight[frame]) != VK_SUCCESS) { throw std::runtime_error("vkQueueSubmit failed"); }
  ...
  fpsFrameCount++;
  float elapsed = std::chrono::duration<float>(std::chrono::high_resolution_clock::now() - fpsStart).count();
  if (elapsed >= 1.0f)
  {
...
void Application::initVulkan()
{
  ...
  createSyncObjects();
  controlPanel.init(window, instance, physDev, dev, qf.graphics, graphicsQ, cmdPool, scFormat, scViews, scExtent, scMinImageCount);
  screenshotMgr.init({ dev, physDev, graphicsQ, cmdPool }, scFormat, scExtent, scImages, storageImg);
  setupKeyBindings();
}
...
void Application::cleanup()
{
  ...
  for (int i = 0; i < MAX_FRAMES; i++)
  {
    vkDestroyBuffer(dev, paramsBufs[i], nullptr);
    vkFreeMemory(dev, paramsMems[i], nullptr);
  ...
  vkDestroyBuffer(dev, tlasScratchBuf, nullptr);
  vkFreeMemory(dev, tlasScratchMem, nullptr);
  vkUnmapMemory(dev, instanceMem);
  vkDestroyBuffer(dev, instanceBuf, nullptr);
  vkFreeMemory(dev, instanceMem, nullptr);
  ...
  vkDestroyBuffer(dev, aabbBuf, nullptr);
  vkFreeMemory(dev, aabbMem, nullptr);
