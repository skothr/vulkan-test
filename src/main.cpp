...
private:
  ...
  void initWindow ()
  {
    ...
    glfwSetFramebufferSizeCallback(window, [](GLFWwindow *w, int, int) {
      ((App*)glfwGetWindowUserPointer(w))->framebufferResized = true;
    });
  }
  ...
  void createInstance ()
  {
    ...
    ai.pApplicationName = "VulkanCube"; ai.applicationVersion = VK_MAKE_VERSION(1,0,0);
    ai.pEngineName = "None"; ai.apiVersion = VK_API_VERSION_1_0;

    uint32_t glfwExtCnt; const char **glfwExts = glfwGetRequiredInstanceExtensions(&glfwExtCnt);
    VkInstanceCreateInfo ci{VK_STRUCTURE_TYPE_INSTANCE_CREATE_INFO};
    ci.pApplicationInfo = &ai;
    ci.enabledExtensionCount = glfwExtCnt; ci.ppEnabledExtensionNames = glfwExts;
  ...
  void createSwapchain ()
  {
    ...
    ci.surface = surface; ci.minImageCount = imgCnt;
    ...
    if (qf.graphics != qf.present) {
      ...
      ci.queueFamilyIndexCount = 2; ci.pQueueFamilyIndices = qis;
                    ...
                    VkImage &img, VkDeviceMemory &mem)
  {
    ...
    ci.mipLevels = 1; ci.arrayLayers = 1; ci.format = fmt; ci.tiling = tiling;
    ci.initialLayout = VK_IMAGE_LAYOUT_UNDEFINED; ci.usage = usage;
    ...
    ai.allocationSize = req.size; ai.memoryTypeIndex = findMemoryType(req.memoryTypeBits, memProps);
  ...
  void createRenderPass ()
  {
    ...
    depth.format  = findDepthFormat(); depth.samples = VK_SAMPLE_COUNT_1_BIT;
    ...
    ci.attachmentCount = atts.size(); ci.pAttachments = atts.data();
  ...
  void createGraphicsPipeline ()
  {
    ...
    auto bind  = Vertex::binding(); auto attrs = Vertex::attribs();
    VkPipelineVertexInputStateCreateInfo vi{VK_STRUCTURE_TYPE_PIPELINE_VERTEX_INPUT_STATE_CREATE_INFO};
    vi.vertexBindingDescriptionCount   = 1;          vi.pVertexBindingDescriptions   = &bind;
    vi.vertexAttributeDescriptionCount = attrs.size(); vi.pVertexAttributeDescriptions = attrs.data();
    ...
    ci.pViewportState      = &vs;  ci.pRasterizationState = &rs;
    ci.pMultisampleState   = &ms;  ci.pDepthStencilState  = &ds;
    ci.pColorBlendState    = &cb;  ci.pDynamicState       = &dyn;
  ...
  void createFramebuffers ()
  {
    ...
    for (size_t i = 0; i < scViews.size(); i++) {
      ...
      ci.renderPass = renderPass; ci.attachmentCount = atts.size(); ci.pAttachments = atts.data();
      ci.width = scExtent.width; ci.height = scExtent.height; ci.layers = 1;
  ...
  void createDescriptorPool ()
  {
    ...
    ci.poolSizeCount = 1; ci.pPoolSizes = &ps; ci.maxSets = MAX_FRAMES;
  ...
  void createDescriptorSets ()
  {
    ...
    ai.descriptorPool = descPool; ai.descriptorSetCount = MAX_FRAMES; ai.pSetLayouts = layouts.data();
  ...
  void recordCommandBuffer (VkCommandBuffer cb, uint32_t imgIdx)
  {
    ...
    rpi.clearValueCount = clears.size(); rpi.pClearValues = clears.data();
  ...
  void drawFrame ()
  {
    ...
    VkSubmitInfo si{VK_STRUCTURE_TYPE_SUBMIT_INFO};
    si.waitSemaphoreCount   = 1; si.pWaitSemaphores   = &imgAvail[frame]; si.pWaitDstStageMask = &waitStage;
    si.commandBufferCount   = 1; si.pCommandBuffers   = &cmdBufs[frame];
