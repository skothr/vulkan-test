...
void Settings::initRegistry()
{
  ...
  registry.add("sphereRadius", &sphereRadius, 0.70f).label("Radius").group("Surface").range(0.1f, 1.5f).visibleWhen(isSphere);
  registry.add("cubeSize", &cubeSize, 0.70f).label("Size").group("Surface").range(0.1f, 1.5f).visibleWhen(isCube);
  registry.add("sphereHeight", &sphereHeight, 0.00f).label("Height").group("Surface").range(-0.5f, 3.0f);
  ...
  registry.addSeparator("Surface", is4D);
  registry.addLabel("Cluster Center", "Surface", is4D);
  registry.add("blobsCenterX", &blobsCenterX, 0.0f).label("X").group("Surface").range(-2.0f, 2.0f).visibleWhen(is4D);
  registry.add("blobsCenterY", &blobsCenterY, 0.0f).label("Y").group("Surface").range(-2.0f, 2.0f).visibleWhen(is4D);
  registry.add("blobsCenterZ", &blobsCenterZ, 0.0f).label("Z").group("Surface").range(-2.0f, 2.0f).visibleWhen(is4D);
  registry.add("blobsDist", &blobsDist, 1.5f).label("Distance").group("Surface").range(0.0f, 4.0f).visibleWhen(is4D);

  registry.addSeparator("Surface", is4D);
  registry.addLabel("Blobby 1", "Surface", is4D);
  registry.add("blob1Mu", &blob1Mu, 1.0f).label("Amplitude").group("Surface").range(0.1f, 4.0f).visibleWhen(is4D);
  registry.add("blob1Sigma", &blob1Sigma, 0.55f).label("Sigma 3D").group("Surface").range(0.05f, 2.0f).format("%.3f").visibleWhen(is4D);
  registry.add("blob1W", &blob1W, 0.0f).label("W (4D amp)").group("Surface").range(-2.0f, 2.0f).visibleWhen(is4D);
  registry.add("blob1SigmaW", &blob1SigmaW, 0.5f).label("Sigma 4D").group("Surface").range(0.05f, 2.0f).format("%.3f").visibleWhen(is4D);

  registry.addSeparator("Surface", is4D);
  registry.addLabel("Blobby 2", "Surface", is4D);
  registry.add("blob2Mu", &blob2Mu, 1.0f).label("Amplitude").group("Surface").range(0.1f, 4.0f).visibleWhen(is4D);
  registry.add("blob2Sigma", &blob2Sigma, 0.55f).label("Sigma 3D").group("Surface").range(0.05f, 2.0f).format("%.3f").visibleWhen(is4D);
  registry.add("blob2W", &blob2W, 0.0f).label("W (4D amp)").group("Surface").range(-2.0f, 2.0f).visibleWhen(is4D);
  registry.add("blob2SigmaW", &blob2SigmaW, 0.5f).label("Sigma 4D").group("Surface").range(0.05f, 2.0f).format("%.3f").visibleWhen(is4D);

  // ── Material ─────────────────────────────────────────────────────────────

  registry.add("ior", &ior, 1.50f).label("IOR").group("Material").range(1.0f, 3.0f);
  registry.add("tintAmount", &tintAmount, 0.25f).label("Tint").group("Material").range(0.0f, 1.0f);
  registry.addColor3("tintColor", &tintR, 0.80f, 0.90f, 1.00f).label("Tint Color").group("Material");

  // ── Sun ──────────────────────────────────────────────────────────────────

  registry.add("sunEnabled", &sunEnabled, false).label("Enabled").group("Sun");
  registry.add("sunAzimuth", &sunAzimuth, 0.588f)
    .label("Azimuth")
    .group("Sun")
    .range(-3.14159f, 3.14159f)
    .format("%.2f rad")
    .visibleWhen(sunOn);
  registry.add("sunElevation", &sunElevation, 0.770f)
    .label("Elevation")
    .group("Sun")
    .range(0.0f, 1.5707f)
    .format("%.2f rad")
  ...
  registry.add("sunConeHalf", &sunConeHalf, 0.080f)
    .label("Disk Size")
    .group("Sun")
    .range(0.01f, 0.40f)
    .format("%.3f rad")
  ...
  registry.add("pointLightX", &pointLightX, 1.5f).label("X").group("Point Light").range(-5.0f, 5.0f).visibleWhen(pointOn);
  registry.add("pointLightY", &pointLightY, 0.0f).label("Y").group("Point Light").range(-5.0f, 5.0f).visibleWhen(pointOn);
  registry.add("pointLightZ", &pointLightZ, 1.5f).label("Z").group("Point Light").range(-1.0f, 5.0f).visibleWhen(pointOn);
  ...
  registry.add("floorScale", &floorScale, 2.0f).label("Scale").group("Floor").range(0.1f, 8.0f);
  registry.add("floorZ", &floorZ, -1.2f).label("Z").group("Floor").range(-3.0f, 0.0f);
  registry.addColor3("floorLightColor", &floorLightR, 0.85f, 0.85f, 0.85f).label("Light Color").group("Floor");
  registry.addColor3("floorDarkColor", &floorDarkR, 0.12f, 0.12f, 0.12f).label("Dark Color").group("Floor");

  // ── Sky ──────────────────────────────────────────────────────────────────

  registry.addColor3("skyHorizon", &skyHorizonR, 0.45f, 0.55f, 0.85f).label("Horizon").group("Sky");
  registry.addColor3("skyZenith", &skyZenithR, 0.08f, 0.15f, 0.45f).label("Zenith").group("Sky");

  // ── Rendering ────────────────────────────────────────────────────────────

  registry.add("shadowsEnabled", &shadowsEnabled, true).label("Shadows").group("Rendering");
  registry.add("causticsEnabled", &causticsEnabled, true).label("Caustics").group("Rendering").sameLine();
  registry.addSeparator("Rendering");
  registry.add("ambient", &ambient, 0.02f).label("Ambient").group("Rendering").range(0.0f, 0.5f);
  registry.add("maxDepth", &maxDepth, 4).label("Max Depth").group("Rendering").range(1, 8);

  // GPU cost estimate
  registry.addCallback("Rendering", [this]() {
  ...
  // Caustics sub-section
  registry.addSeparator("Rendering");
  registry.addLabel("Caustics", "Rendering");
  ...
  // Soft shadows sub-section
  registry.addSeparator("Rendering");
  registry.addLabel("Soft Shadows", "Rendering");
  ...
  // 4D surface sub-section
  registry.addSeparator("Rendering");
  registry.addLabel("4D Surface", "Rendering");
  ...
  registry.add("fov", &fov, 45.0f).label("FOV").group("Camera").range(10.0f, 120.0f).format("%.1f\xc2\xb0");
  registry.add("sensitivity", &sensitivity, 0.005f).label("Sensitivity").group("Camera").range(0.001f, 0.02f).format("%.3f");
  registry.add("zoomSpeed", &zoomSpeed, 0.1f).label("Zoom Speed").group("Camera").range(0.01f, 0.5f);
  ...
  registry.add("rotAxisX", &rotAxisX, 0.5f).label("Axis X").group("Animation").range(-1.0f, 1.0f);
  registry.add("rotAxisY", &rotAxisY, 1.0f).label("Axis Y").group("Animation").range(-1.0f, 1.0f);
  registry.add("rotAxisZ", &rotAxisZ, 0.0f).label("Axis Z").group("Animation").range(-1.0f, 1.0f);
